...
{
    push_trace("uint32_t ext2_get_file_size(FILE*)", "ext2_get_file_size", file,
        0, 0, 0);
    inode_base_t inode;
...
{
    push_trace("uint32_t ext2_mkdir(FILE*,char*,uint16_t)", "ext2_mkdir", file,
        name, perm, 0);
...
{
    push_trace("void ext2_mkfile(FILE*,char*,uint16_t)", "ext2_mkfile", file,
        name, perm, 0);
    uint32_t index = alloc_inode();
...
{
    push_trace(
        "void ext2_unlink(FILE*,char*)", "ext2_unlink", file, name, 0, 0);
...
char** ext2_listdir(FILE* file)
{
    ...
    while (cur_offset < p_inode->size_lower) {
        ...
        uint32_t expected_size
            = ((sizeof(dirent_t) + cur_dir->name_len) & 0xfffffffc) + 0x4;
...
{
    push_trace("FILE *file_from_dirent(dirent_t*,inode_bast_t*)",
        "file_from_dirent", dir, inode, 0, 0);
    FILE* f = NULL;
    new(f, 1, FILE);
...
{
    push_trace(
        "FILE *ext2_finddir(FILE*,char*)", "ext2_finddir", file, name, 0, 0);
    inode_base_t* p_inode = NULL;
    new(p_inode, 1, inode_base_t);
    read_inode_metadata(p_inode, file->inode);
    uint32_t expected_size;
    ...
    while (cur_offset < p_inode->size_lower) {
        ...
        if (((sizeof(dirent_t) + cur_dir->name_len) & 0x3) != 0) {
            ...
            expected_size
                = ((sizeof(dirent_t) + cur_dir->name_len) & 0xfffffffc);
        }
...
{
    push_trace("void ext2_create_entry(FILE*,char*,uint32_t)",
        "ext2_create_entry", file, name, inode, 0);
    inode_base_t* p_inode = NULL;
    new(p_inode, 1, inode_base_t);
    read_inode_metadata(p_inode, file->inode);
    uint32_t cur_offset = 0;
    ...
    while (cur_offset < p_inode->size_lower) {
        ...
        if (found) {
            ...
            cur_dir->type = 0;
            // Must use memcpy instead of strcpy, because name in direntry does
            // not contain ending '\0'
            memcpy(cur_dir->name, name, strlen(name));
            write_inode_block(p_inode, block_offset, block_buf);
            // Then, append a new ending entry
            // Be careful about a spcial case here, if not enough space for
            // ending entry, put it in the new block
            in_block_offset += cur_dir->size;
            if (in_block_offset >= fs->block_size) {
                block_offset++;
                in_block_offset = 0;
...
{
    push_trace("void ext2_remove_entry(FILE*,char*)", "ext2_remove_entry", file,
        name, 0, 0);
    inode_base_t* p_inode = NULL;
    new(p_inode, 1, inode_base_t);
    read_inode_metadata(p_inode, file->inode);
    ...
    char* block_buf = read_inode_block(p_inode, block_offset);
    // Note: It is required that no directory entry cross block boundary && each
    // directory entry be aligned on 4-byte boundary
    while (cur_offset < p_inode->size_lower) {
        if (in_block_offset >= fs->block_size) {
            block_offset++;
            in_block_offset = 0;
...
{
    push_trace(
        "void ext2_chmod(FILE*,uint32_t)", "ext2_chmod", file, mode, 0, 0);
    inode_base_t* inode = NULL;
    new(inode, 1, inode_base_t);
    read_inode_metadata(inode, file->inode);
...
{
    push_trace("uint32_t ext2_read(FILE*,uint32_t,uint32_t,char*)", "ext2_read",
        file, offset, size, buf);
...
{
    push_trace("uint32_t ext2_write(FILE*,uint32_t,uint32_t,char*)",
        "ext2_write", file, offset, size, buf);
    // Extract the ext2 filesystem object and inode from vfs node
    inode_base_t* inode = NULL;
    new(inode, 1, inode_base_t);
    read_inode_metadata(inode, file->inode);
...
{
    push_trace(
        "void ext2_open(FILE*,uint32_t)", "ext2_open", file, flags, 0, 0);
...
void write_superblock(void) { write_disk_block(1, (void*)fs->superblock); }

uint32_t ext2_alloc_block(void)
{
    push_trace(
        "uint32_t ext2_alloc_block(void)", "ext2_alloc_block", 0, 0, 0, 0);
    uint32_t* buf = NULL;
...
{
    push_trace(
        "void ext2_free_block(uint32_t)", "ext2_free_block", block, 0, 0, 0);
    uint32_t* buf = NULL;
    new(buf, fs->block_size, uint32_t);
...
{
    push_trace(
        "FILE *get_ext2_root(inode_base_t*)", "get_ext2_root", inode, 0, 0, 0);
...
{
    push_trace("void scan_superblock(uint32_t)", "scan_superblock",
        super_sector, 0, 0, 0);
    superblock_t s;
    s.signature = 0;
    ...
    if ((s.signature != 0xef53)) {
        printk(
            RED("EXT2: Superblock invalid or not found: %8x\n"), s.signature);
    }
    printk("\t\tSIG: %x\n", s.signature);
    pop_trace();
}
...
{
    push_trace(
        "void initial_fs(struct block_device)", "initial_fs", dev, 0, 0, 0);
    int ret = E_NOERR;
    new(fs, 1, EXT2_t);
    new(fs->pwd, 2, char);
...
int read_BGD(void)
{
    ...
    uint32_t n_sectors
        = ((sizeof(block_group_descriptor_t) * fs->n_groups) / 512) + 1;
    uint32_t bgd_block = ((2048) / fs->block_size) + 1;
    uint32_t n_blocks
        = (fs->n_groups * sizeof(block_group_descriptor_t) / fs->block_size)
        + 1;
    ...
    for (int i = 0; i < n_blocks; i++) {
        fs->dev->read(((uint8_t*)fs->BGD + fs->block_size * i),
            fs->block_size / 512, fs->start_block + (bgd_block + i) * 8);
    }

    // fs->dev->read((uint8_t *)fs->BGD, n_sectors, fs->start_block +
    // bgd_sector);
...
(int64_t)(((int64_t)(root_inode->f_size_dir_acl <<\ 32)) |
root_inode->size_lower)); printk(CYAN("Inode: perms: %4x\n"),
...
int populate_recur(fs_tree* start)
{
    ...
    for (int i = 0; (i < n_blocks) & (i < 12); i++) {
        ...
        fs->dev->read(raw_block + fs->block_size * i, 8,
            fs->start_block + (cur_inode->blocks[i] * 8));
    }
    ...
    */
    ...
    do {
        ...
        case FS_DIR:
            ...
            tdir->n_entries = 0;
            if ((strncmp(tdir->name, ".", 1) != 0)
                & (strncmp(tdir->name, "..", 1) != 0)) {
                printk(YELLOW("RECURSING\n"));
                ret = populate_recur(tdir);
                if (ret != E_NOERR) {
                    printk(
                        RED("VFS: failed to populate in recurrence %x\n"), ret);
                    goto exit;
                }
            }
            break;
        default:
            ...
            printk(
                YELLOW("Unknown file type %d: placing dummy\n"), tmp_dir->type);
            break;
        }

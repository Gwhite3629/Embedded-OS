...
#include <drivers/ethernet/mii.h>
#include <drivers/ethernet/eth.h>
...
static int bcm54xx_ack_intr(struct phy_device *phydev)
{
    int reg;
...
int bcm54xx_config_intr(struct phy_device *phydev)
{
    ...
    if (phydev->interrupts == 0) {
        err = bcm54xx_ack_intr(phydev);
        if (err)
            return err;
        ...
        err = bcm54xx_phy_ack_intr(phydev);
    }

    return err;
}

int bcm54xx_handle_interrupt(struct phy_device *phydev)
{
...
int phydev_init(struct phy_device **phydev)
{
    new(*phydev, 1, struct phy_device);
    new((*phydev)->drv, 1, struct phy_driver);

    drv->soft_reset = NULL;
    drv->config_init = &bcm54xx_config_init;
    drv->probe = &bcm54xx_probe;
    drv->suspend = &bcm54xx_suspend;
    drv->resume = &bcm54xx_resume;
    drv->config_aneg = NULL;
    drv->read_status = NULL;
    drv->config_intr = &bcm54xx_config_intr;
    drv->handle_interrupt = &bcm54xx_handle_interrupt;
    drv->remove = NULL;
    drv->link_change_notify = bcm54xx_link_change_notify;

    bcm54xx_probe(*phydev);

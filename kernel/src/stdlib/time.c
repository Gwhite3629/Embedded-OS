...
int timer_init(void)
{
    ...
    chip_write(TIMER_CONTROL_32BIT | TIMER_CONTROL_ENABLE
            | TIMER_CONTROL_INT_ENABLE | TIMER_CONTROL_PRESCALE_256,
        TIMER_CONTROL);
...
void wait_msec(unsigned int n)
{
    ...
    // Get current counter
    asm volatile("mrs %[result], cntpct_el0" : [result] "=r"(t));
    unsigned long i = ((f / 1000) * n) / 1000;
    // printk("WAIT: %x, %x, %x\n", f, t, i);

    do {
        asm volatile("mrs %[result], cntpct_el0" : [result] "=r"(r));
    } while ((r - t) < i);
    // printk("DONE WAIT\n");
}

void noinline sys_timer_init(void)
{
...
uint64_t noinline sys_timer_read(void)
{

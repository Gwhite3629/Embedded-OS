...
void uart_init(void) {
	...
	old=readl(GPIO_GPFSEL1);
	old &= ~(0x7 << 12);
	old |= (4<<12);

	old &= ~(0x7 << 15);
	old |= (4<<15);
	writel(old, GPIO_GPFSEL1);
	...
	writel(GPIO_GPPUD_DISABLE, GPIO_GPPUD);
	delay(150);

	/* Pass the disable clock to GPIO pins 14 and 15 and delay*/
	writel((1 << 14) | (1 << 15), GPIO_GPPUDCLK0);
	delay(150);

	/* Write 0 to GPPUDCLK0 to make it take effect */
	writel(0x0, GPIO_GPPUDCLK0);
	...
	writel(UART0_LCRH_FEN | UART0_LCRH_WLEN_8BIT, UART0_LCRH);

	/* Enable UART0, receive, and transmit */
	writel(UART0_CR_UARTEN |
				UART0_CR_TXE |
				UART0_CR_RXE,
				UART0_CR);
}
...
void uart_putc(unsigned char byte) {
	...
	while ( readl(UART0_FR) & UART0_FR_TXFF ) {
	}
...
unsigned char uart_getc(void) {
	...
	while ( readl(UART0_FR) & UART0_FR_RXFE ) {
	}
...
int32_t uart_getc_noblock(void) {
	...
	if ( readl(UART0_FR) & UART0_FR_RXFE ) {
		return 0;
	}
